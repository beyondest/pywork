...
def trainning(dataset, labelset, weight1, weight2, value1, value2):
	...
	for i in range(len(dataset)):
		...
		#隐层输入,only one line one time,size matched,cause x=len(dataset[0])
		input1 = np.dot(inputset, weight1).astype(np.float64)
		#隐层输出,minus threshold then activate
...
def testing(dataset, labelset, weight1, weight2, value1, value2):
	...
	for i in range(len(dataset)):
		...
		#确定其预测标签,this is classifacation prediction
		if output3 >0.5 :
			flag = 2
		else:
			flag = 1
