...
def auto_aim(img):
    ...
    if mode == "BLUE":
        ...
        # # HSV 红色
        # lower_red = np.array([0, 79, 49])
        # upper_red = np.array([12, 255, 255])
        # mask0 = cv2.inRange(hsv_image, lower_red, upper_red)
        # # 区间2
        # lower_red = np.array([152, 37, 105])
    ...
    if n >= 2:  # 图像中找到两个以上的灯条
        for count in range(0, n):
            ...
            while findCache < n:  # 未超界，进行匹配运算
                ...
                calcCache = (2 * calcCache) / (longSide[count] + longSide[findCache])  # 装甲板矩形长宽比比值
    ...
    if real_rectbox:
        ...
        if debug:
            theta_x, theta_y = rotateXYZtoYAWPITCH(predicted_X, predicted_Y, predicted_Z)
